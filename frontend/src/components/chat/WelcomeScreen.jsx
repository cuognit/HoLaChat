...
export default function WelcomeScreen() {
    ...
    return (
        <div className="bg-white flex-1 flex flex-col items-center justify-center h-screen w-full relative overflow-hidden">
            ...
            <div className="absolute top-20 flex flex-col items-center text-center px-4 w-full">
                <h1 className="text-3xl text-gray-800 mb-5">
                    Chào mừng đến với <span className="font-bold text-blue-600">HoLa Chat!</span>
                </h1>
                <p className="text-gray-700 text-[15px] max-w-md leading-relaxed text-center">
            ...
            <div className="flex items-center justify-between w-full px-12 mt-24">
                
                {/* Nút mũi tên trái */}
                <button 
                    onClick={prevSlide}
                    className="cursor-pointer p-2 text-blue-500 hover:bg-gray-100 hover:text-blue-600 rounded-full transition-colors flex-shrink-0"
                >
                    <ChevronLeft size={40} strokeWidth={1.5} />
                </button>

                {/* Viewport cho Slide */}
                <div className="flex-1 overflow-hidden relative min-h-[400px] flex items-center">
                        ...
                        {carouselData.map((slide) => (
                            <div key={slide.id} className="w-full flex-shrink-0 flex flex-col items-center px-8">
                                ...
                                <img 
                                    src={slide.image} 
                                    alt={slide.title} 
                                    className="w-[350px] h-[250px] object-contain mb-10"
                                />
                                
                                {/* Tiêu đề chức năng */}
                                <h2 className="text-xl font-medium text-blue-600 mb-3 text-center">
                                    {slide.title}
                                </h2>
                                
                                {/* Mô tả chức năng */}
                                <p className="text-gray-600 text-[15px] text-center max-w-[500px]">
                                    {slide.description}
                                </p>
                            </div>
                        ))}
                ...
                <button 
                    onClick={nextSlide}
                    className="p-2 text-blue-500 hover:bg-gray-100 hover:text-blue-600 rounded-full transition-colors flex-shrink-0 cursor-pointer"
                >
