        ...
        selectedIds.forEach(id => {
            ...
            try {
                ...
                if (extraMessage.trim()) {
                    ...
                    else if (room.targetUserId) extraPayload.receiverId = room.targetUserId;
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[450px] bg-white rounded-xl shadow-2xl flex flex-col max-h-[85vh] animate-scale-up">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    ...
                    <button onClick={onClose} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-full transition-colors cursor-pointer">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex px-5 border-b border-gray-100 gap-6 mt-2">
                    ...
                    >
                        Gần đây
                        {activeTab === 'gần-đây' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-t-full" />}
                    </button>
                    <button 
                    ...
                    >
                        Nhóm trò chuyện
                        {activeTab === 'nhóm' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-t-full" />}
                    </button>
                    <button 
                    ...
                    >
                        Bạn bè
                        {activeTab === 'bạn-bè' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-t-full" />}
                    </button>
                </div>
                    ...
                    {isLoading ? (
                        ...
                        filteredRooms.map(room => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-5 h-5 rounded-md flex items-center justify-center border transition-colors shrink-0 ${isSelected ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-300'}`}>
                                        {isSelected && <Check className="w-3.5 h-3.5 text-white stroke-[3]" />}
                                    </div>
                                    <img 
                                        src={room.isGroup ? room.avatarUrl : (room.targetAvatarUrl || room.avatarUrl || '/avatar.jpg')} 
                                        alt="" 
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-100">
                    <div className="bg-white p-3 rounded-lg border border-gray-200 mb-3 flex gap-3 items-start shadow-sm">
                        <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center shrink-0">
                            {messageData?.messageType === 'IMAGE' ? (
                                <img 
                                    src={Array.isArray(JSON.parse(messageData.content || "[]")) ? JSON.parse(messageData.content)[0] : messageData.content} 
                                    className="w-full h-full object-cover rounded-lg"
                                    alt=""
                                />
                            ) : (
                                <span className="font-bold text-xs">A<span className="lowercase">a</span></span>
                            )}
                        </div>
                        <div className="flex-1 min-w-0">
                            <h4 className="text-[13px] font-bold text-gray-800">Chia sẻ {messageData?.messageType === 'IMAGE' ? 'hình ảnh' : 'tin nhắn'}</h4>
                            <p className="text-xs text-gray-500 truncate mt-0.5">
                                {messageData?.messageType === 'IMAGE' ? '[Hình ảnh]' : messageData?.content}
                            </p>
                        </div>
                    </div>
                    ...
                    <input
                        ...
                        className="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                    />
                </div>
