...
export default function Message({
  ...
  const isLastInConsecutiveGroup = () => {
    ...
    try {
      ...
      const diffMinutes =
        Math.abs(nextTime.getTime() - currentTime.getTime()) / (1000 * 60);
  ...
  const isConsecutive = () => {
    ...
    try {
      ...
      const diffMinutes =
        Math.abs(currentTime.getTime() - prevTime.getTime()) / (1000 * 60);
      return diffMinutes <= 1;
    } catch (error) {
  ...
  const shouldShowTime = () => {
    ...
    if (
      String(currentSenderId) === String(nextMessageSenderId) &&
      nextMessageCreatedAt
    ) {
      try {
        const currentTime = safeParseDate(currentMessageCreatedAt);
  ...
  const shouldShowSenderName =
    isGroup && !isSentByMe && senderName && !isConsecutive();
  ...
  return (
    <>
      <div
        id={`msg-${messageId}`}
        className={`flex items-start gap-2 w-full ${getMarginTop()} ${isSentByMe ? "justify-end" : "justify-start"}`}
      >
        {!isSentByMe &&
          (isConsecutive() ? (
        ...
        <div
          className={`flex flex-col flex-1 max-w-[85%] ${isSentByMe ? "items-end" : "items-start"}`}
        >
          <div
            className={`flex items-center gap-2 group/msg relative ${isSentByMe ? "flex-row-reverse" : "flex-row"}`}
          >
            <div
              className={`relative ${isRecalled ? "px-4 py-2" : messageType === "IMAGE" ? "p-1" : "px-4 py-2"} rounded-2xl shadow-sm border max-w-lg transition-all duration-150 ${
                isRecalled
            ...
            >
              {messageData?.forwarded && !isRecalled && (
                <div
                  className={`flex items-center gap-1 mb-1 opacity-70 ${messageType === "IMAGE" ? "px-3 pt-1" : ""}`}
                >
              ...
              {shouldShowSenderName && !isRecalled && (
                <p
                  className={`text-[11px] font-semibold text-orange-600 mb-0.5 select-none ${messageType === "IMAGE" && !messageData?.forwarded ? "px-3 pt-1" : ""}`}
                >
                  {senderName}
                </p>
              )}
                ...
                >
                  <div className="flex-1 min-w-0 flex flex-col pr-2">
                    <span className="text-[11px] font-semibold text-blue-600">
                      {messageData.replyToSenderName || "Người dùng"}
                    </span>
                    {messageData.replyToMessageType === "IMAGE" ? (
                        ...
                        {(() => {
                          ...
                          return (
                            <img
                              src={imgUrl}
                              alt="Reply Thumbnail"
                              className="w-6 h-6 object-cover rounded-[3px] border border-gray-200"
                            />
                          );
                        })()}
                    ...
                    ) : messageData.replyToMessageType === "VIDEO" ? (
                      <div className="flex items-center gap-1 mt-0.5">
                        <video
                          src={messageData.replyToContent}
                          className="w-6 h-6 object-cover rounded-[3px] border border-gray-200 bg-black"
                        />
                        <span className="text-[12px] text-gray-500 italic truncate">
                          [Video]
                        </span>
                      </div>
                    ) : messageData.replyToMessageType === "FILE" ? (
                      (() => {
                        let fileName = "File đính kèm";
                        try {
                          const parsed = JSON.parse(messageData.replyToContent);
                          if (parsed.name) fileName = parsed.name;
                        } catch {}
                        return (
                          <div className="flex items-center gap-1 mt-0.5">
                            <div className="w-6 h-6 bg-blue-50 border border-gray-200 rounded-[3px] flex items-center justify-center shrink-0">
                              <FileText className="w-4 h-4 text-blue-500" />
                            </div>
                            <span className="text-[12px] text-gray-500 italic truncate max-w-[150px]">
                              [{fileName}]
                            </span>
                          </div>
                        );
                      })()
                    ) : messageData.replyToMessageType === "CALL" ? (
                      <div className="flex items-center gap-1 mt-0.5">
                        <span className="text-[12px] text-gray-500 italic truncate">
                          [Cuộc gọi thoại]
                        </span>
                      ...
                      <p className="text-[12px] text-gray-600 truncate opacity-90">
                        {messageData.replyToContent}
                      </p>
                    )}
                  </div>
              ...
              {isRecalled ? (
                <div className="flex items-center gap-2 select-none">
                  ...
                  <p className="text-[13.5px] text-gray-400 italic">
                ...
                <video
                  src={content}
                  controls
                  className="max-w-[180px]  rounded-lg shadow-sm border border-gray-100"
                />
              ) : messageType === "FILE" ? (
                (() => {
                  let fileData;
                ...
                (() => {
                  ...
                  try {
                    ...
                    callData = { callStatus: "UNKNOWN", duration: 0, callType: "AUDIO" };
                  }
                  const status = callData.callStatus;
                  const dur = callData.duration || 0;
                  const callType = callData.callType || "AUDIO";
                  const isMissed = status === "MISSED" || status === "REJECTED" || status === "CANCELLED";
                  ...
                  return (
                    <div className={`flex items-center gap-3 p-2 rounded-lg border shadow-sm min-w-[200px] ${isMissed ? "bg-red-50/50 border-red-100" : "bg-white/70 border-gray-200"}`}>
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${isMissed ? "bg-red-100 text-red-500" : "bg-green-100 text-green-600"}`}>
                        {callType === "VIDEO" ? (
                      ...
                      <div className="flex flex-col">
                        <span className={`text-sm font-medium ${isMissed ? "text-red-600" : "text-gray-800"}`}>
                          {isMissed ? "Cuộc gọi nhỡ" : (callType === "VIDEO" ? "Cuộc gọi Video" : "Cuộc gọi thoại")}
                        </span>
                        {!isMissed && dur > 0 && (
                          <span className="text-[11px] text-gray-500 mt-0.5">
                            {formatDur(dur)}
                ...
                <p className="wrap-break-word whitespace-pre-wrap leading-relaxed text-[14.5px]">
                  {content}
                </p>
              )}
              ...
              {shouldShowTime() && (
                <p
                  className={`text-[10px] text-gray-400 mt-2 select-none ${!isRecalled && messageType === "IMAGE" ? "px-3 pb-1" : ""} ${isSentByMe ? "text-right" : "text-left"}`}
                >
                  {time}
                </p>
              )}
          ...
          {copySuccess && (
            <div
              className={`text-[11px] text-green-600 font-medium mt-1 animate-in fade-in duration-200 ${isSentByMe ? "text-right" : "text-left"}`}
            >
              ✓ Đã sao chép
            </div>
          )}
          ...
          {showSentStatus && (
            <div
              className="flex items-center gap-1 mt-3 text-gray-400"
              title="Đã gửi"
            >
