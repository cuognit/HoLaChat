import { AlarmClock, Users, ChevronDown, ChevronUp, Clock, HelpCircle, EyeOff, Pencil, Crown, UserPlus, LogOut, Trash2, Check, X, Link, Loader2 } from 'lucide-react';
...
export default function ConversationInfo() {
    ...
    const [openSections, setOpenSections] = useState({ media: false, file: false, link: false, security: true });
        ...
        const sub = subscribe(`/topic/room/${roomId}/members`, (message) => {
            try {
                ...
                if (type === "ROLE_CHANGED") {
                    setMembers(prev => prev.map(m => m.userId === data.userId ? { ...m, role: data.role } : m));
                    // Cập nhật myRole nếu là mình
                    if (data.userId === currentUser?.id) {
                        setSelectedUser(prev => prev ? { ...prev, currentUserRole: data.role } : prev);
                    }
                }
            } catch (e) {}
        });
    ...
    const handleSaveName = async () => {
        if (!nameInput.trim() || nameInput.trim() === (selectedUser.roomName || selectedUser.targetUserName)) {
            setEditingName(false);
            return;
        }
        setIsSavingName(true);
        try {
            await api.patch(`/chat-rooms/${roomId}/name`, null, { params: { name: nameInput.trim() } });
            setSelectedUser(prev => prev ? { ...prev, roomName: nameInput.trim() } : prev);
    ...
    const handleCopyLink = async () => {
        ...
        try {
            ...
            toast.success("Đã sao chép link mời!");
        } catch { toast.error("Không thể lấy link mời"); }
    ...
    const handleRoleChange = async (targetUserId, newRole) => {
        try {
            ...
            if (updated) {
                ...
                toast.success(newRole === "ADMIN" ? "Đã nâng lên quản trị viên" : "Đã hạ xuống thành viên");
            }
        } catch { toast.error("Thao tác thất bại"); }
    };
    ...
    const handleLeave = () => {
        setConfirmConfig({
            isOpen: true,
            title: "Rời khỏi nhóm",
            message: "Bạn có chắc chắn muốn rời khỏi nhóm này? Các thành viên khác vẫn tiếp tục trò chuyện.",
            type: "warning",
    ...
    const handleDissolve = () => {
        setConfirmConfig({
            isOpen: true,
            title: "Giải tán nhóm",
            message: "Giải tán nhóm? Tất cả tin nhắn và dữ liệu nhóm sẽ bị xóa vĩnh viễn khỏi hệ thống!",
            type: "danger",
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    navigate("/");
                } catch { toast.error("Giải tán nhóm thất bại"); }
            }
        });
    };
            ...
            {isGroup && (
                <div className="flex border-b border-gray-200 shrink-0">
                    {["info", "members"].map(tab => (
                        ...
                        >
                            {tab === "info" ? "Thông tin" : `Thành viên${members.length > 0 ? ` (${members.length})` : ""}`}
                        </button>
                    ))}
                </div>
            )}
            ...
            {activeTab === "info" && (
                <div className="flex-1 overflow-y-auto">
                    ...
                    <div className="flex flex-col items-center py-5 border-b border-gray-100">
                        ...
                        {isGroup ? (
                            displayAvatar
                                ? <img src={displayAvatar} alt={displayName} className="w-16 h-16 rounded-full object-cover border border-gray-200 mb-3" />
                        ...
                        {editingName ? (
                            <div className="flex items-center gap-1.5 w-full px-6">
                                <input
                                    ...
                                    autoFocus
                                />
                                <button onClick={handleSaveName} disabled={isSavingName} className="p-1 text-green-600 hover:bg-green-50 rounded cursor-pointer">
                                    {isSavingName ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}
                                </button>
                                <button onClick={() => setEditingName(false)} className="p-1 text-gray-400 hover:bg-gray-100 rounded cursor-pointer">
                                    <X size={14} />
                                </button>
                            </div>
                        ) : (
                            <div className="flex items-center gap-2">
                                <span className="text-[17px] font-medium text-gray-900">{displayName}</span>
                                <button
                    ...
                    <div className="flex flex-col py-2 border-b border-gray-100">
                        ...
                        {isGroup && (
                            ...
                            >
                                {isLoadingLink ? <Loader2 className="text-gray-400 animate-spin" size={20} /> : <Link className="text-gray-600" size={20} strokeWidth={1.5} />}
                    ...
                    <div className="flex-1">
                        ...
                        <div className="border-b border-gray-100">
                            <button onClick={() => toggleSection('media')} className="flex items-center justify-between w-full px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer">
                        ...
                        {/* File */}
                        <div className="border-b border-gray-100">
                            <button onClick={() => toggleSection('file')} className="flex items-center justify-between w-full px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer">
                        ...
                        {/* Security */}
                        <div className="border-b border-gray-100">
                            <button onClick={() => toggleSection('security')} className="flex items-center justify-between w-full px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer">
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {isAdmin && (
                        <div className="px-3 pt-3 pb-1">
                            <AddMemberInline roomId={roomId} onAdded={(newMembers) => {
                                setMembers(prev => {
                                    const existingIds = new Set(prev.map(m => m.userId));
                                    const fresh = newMembers.filter(m => !existingIds.has(m.userId));
                                    return [...prev, ...fresh];
                                });
                            }} />
                        </div>
...
// Inline component thêm thành viên
function AddMemberInline({ roomId, onAdded }) {
    ...
    const handleSearch = (e) => {
        ...
        timerRef.current = setTimeout(async () => {
            try {
                const res = await api.get("/auth/search", { params: { email: q.trim() } });
    ...
    if (!open) {
        return (
            <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-sm font-semibold text-blue-600 hover:bg-blue-50 border border-dashed border-blue-300 rounded-xl transition-colors cursor-pointer"
            >
    ...
    return (
        <div className="border border-blue-200 rounded-xl p-2.5 bg-blue-50/50">
            <div className="flex items-center bg-white border border-gray-200 rounded-lg px-2.5 py-1.5 gap-2 mb-2">
                <input
                    autoFocus
                    value={query}
                ...
                <button onClick={() => { setOpen(false); setSelected([]); setQuery(""); setResults([]); }} className="text-gray-400 hover:text-gray-600 cursor-pointer">
                    <X size={14} />
                </button>
            </div>
                ...
                <div className="max-h-36 overflow-y-auto space-y-0.5 mb-2">
                    {results.map(u => (
                        <div key={u.id} onClick={() => toggle(u)}
                            className={`flex items-center gap-2 p-1.5 rounded-lg cursor-pointer transition-colors ${isSelected(u.id) ? "bg-blue-100" : "hover:bg-white"}`}>
                            <img src={u.avatarUrl || "/avatar.jpg"} alt={u.userName} className="w-7 h-7 rounded-full object-cover" />
                            <span className="text-sm font-medium text-gray-800 flex-1">{u.userName}</span>
                            {isSelected(u.id) && <Check size={14} className="text-blue-600" />}
                        </div>
                    ))}
                </div>
            )}
            ...
            {selected.length > 0 && (
                <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-500">Đã chọn {selected.length} người</span>
                    <button
                        onClick={handleAdd}
                        disabled={isAdding}
                        className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1"
                    >
