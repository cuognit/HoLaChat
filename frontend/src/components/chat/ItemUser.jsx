...
export default function ItemUser({
  ...
  const getLastMessagePreview = () => {
    ...
    let content = user.lastMessage || "Hãy gửi lời chào ngay!";

    // Định dạng lại nội dung hiển thị theo từng loại tin nhắn
    if (msgType === "IMAGE") {
      content = "📷 Hình ảnh";
    } else if (msgType === "VIDEO") {
      content = "🎬 Video";
    } else if (msgType === "FILE") {
      // Nếu content đã được format sẵn ở SendChat (bắt đầu bằng 📎) thì giữ nguyên
    ...
    // Private
    const senderName =
      String(user.lastSenderId) === String(currentUserId)
        ? currentUserName
        : targetUserName;
    if (String(user.lastSenderId) === String(currentUserId)) {
      return `Bạn: ${content}`;
          ...
          {typingUsers.length > 0 ? (
            <p className="truncate text-sm max-w-[220px] text-blue-500 font-medium flex items-center gap-1">
              <span className="flex items-center gap-0.5">
                <span
                  className="typing-dot-sm w-[4px] h-[4px] bg-blue-500 rounded-full inline-block"
